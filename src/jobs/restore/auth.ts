  ...
  constructor(settings: Settings, admin: App, store: Storage) {
    ...
    this.writeStream = new Writable({
      write(object: DataModel, encoding, callback) {
        (async () => {
          ...
          else userData = AuthConverter.fromString(object.data, false);
    ...
    this.writeBuffer = {
      ...
      commit: async () => {
        ...
        else res = await self.auth.importUsers(self.writeBuffer.batch);
        self.counter += res.successCount;
        if (res.failureCount !== 0) Logger.warn(JSON.stringify(res.errors));
