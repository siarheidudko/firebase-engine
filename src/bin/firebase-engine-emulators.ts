...
function errorHandler() {
  Logger.log("=========================EXAMPLES=========================");
  Logger.log(
    'firebase-engine-emulators path="./test/utils/vend-park-development.json" --to-emulators'
  );
  Logger.log(
    'firebase-engine-emulators p="./test/utils/vend-park-development.json" -from'
  );
...
const run = async () => {
  ...
  process.argv.forEach((val) => {
    ...
    if (val.match(/^--to-emulators/i) || val.match(/^-to/i)) {
      if (typeof settings.toEmulator === "boolean")
        throw new Error(
          "Please use only one param --to-emulators (-to) or --from-emulators (-from)."
        );
  ...
  if (typeof settings.toEmulator !== "boolean")
    throw new Error(
      "Param --to-emulators (-to) or --from-emulators (-from) not set."
    );
  const backupPath = join(tmpdir(), `${randomStr(8)}.backup`);
