...
export class FirestoreConverter {
  ...
  private static toObject(d: any) {
    if (typeof d === "string") return { data: d, type: "string" };
    if (typeof d === "number") return { data: d, type: "number" };
    if (typeof d === "boolean") return { data: d, type: "boolean" };
    if (d === null) return { data: d, type: "null" };
    if (d instanceof Buffer)
      return { data: d.toString("base64"), type: "binary" };
    if (d instanceof Timestamp)
      return {
        data: { sec: d.seconds, nano: d.nanoseconds },
        type: "timestamp",
      };
    if (d instanceof DocumentReference)
      return { data: d.path, type: "document" };
    if (d instanceof GeoPoint)
    ...
    if (Array.isArray(d)) {
      ...
      for (const val of d) _arr.data.push(FirestoreConverter.toObject(val));
      return _arr;
    }
  ...
  private static fromObject(d: any) {
    ...
    switch (d.type) {
      ...
      case "timestamp":
        return new Timestamp(d.data.sec, d.data.nano);
      case "document":
        return getFirestore().doc(d.data);
      case "geopoint":
        return new GeoPoint(d.data.lat, d.data.long);
      case "array":
